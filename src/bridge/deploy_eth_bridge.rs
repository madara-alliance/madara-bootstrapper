...
pub async fn deploy_eth_bridge(
    ...
    log::info!("Eth Bridge Deployment Successful [✅]");
    log::info!("[🚀] Eth Bridge Address : {:?}", eth_bridge.address());
    ...
    log::info!("L2 Bridge Deployment Successful [✅]");
    log::info!("[🚀] L2 Bridge Address : {:?}", l2_bridge_address);
    ...
    log::info!("[🚀] ETH Bridge initialized");

    sleep(Duration::from_secs(arg_config.l1_wait_time.parse().unwrap())).await;

    eth_bridge
        .setup_l2_bridge(clients.provider_l2(), l2_bridge_address, eth_address, &arg_config.rollup_priv_key, &account)
        .await;
    log::info!("ETH Bridge L2 setup complete [✅]");
    ...
    eth_bridge
        ...
        .await;
    log::info!("ETH Bridge L1 setup complete [✅]");
